...
from .continuous_event_datasets import RawEventsDataset
from skimage import io
from os.path import join
import numpy as np
from .utils.util import first_element_greater_than, events_to_voxel_grid, normalize_voxelgrid, rgb2gray
import random
import glob
import torch
import cv2
...
class SynchronizedFramesEventsRawDataset(Dataset):
    ...
    def __init__(self, base_folder, event_folder,
                 depth_folder='frames', frame_folder='rgb', 
                 flow_folder='flow', semantic_folder='semantic',
    ...
    def __getitem__(self, i, seed=None, reg_factor=5.70378):
        ...
        voxelgrid = events_to_voxel_grid(events_overall, self.nbr_of_bins, self.height, self.width)
        ...
        frame = 1.0 + np.log(frame+self.eps) / reg_factor
...
class SynchronizedFramesEventsVoxelDataset(Dataset):
...
class EventsVoxelDataset(Dataset):
...
class SequenceSynchronizedFramesEventsDataset(Dataset):
    ...
    def __init__(self, base_folder, event_folder, depth_folder='frames', frame_folder='rgb', flow_folder='flow', semantic_folder='semantic/data/',
        ...
        if dataset_type == 'raw':
                ...
                self.dataset = SynchronizedFramesEventsVoxelDataset(base_folder, event_folder, depth_folder, frame_folder,
                                                           flow_folder, semantic_folder, start_time, stop_time,
                                                           clip_distance, every_x_rgb_frame, transform,
                                                           normalize=normalize, use_phased_arch=use_phased_arch,
                                                           baseline=baseline, loss_composition=loss_composition,
                                                           reg_factor=reg_factor)
        elif dataset_type == 'voxeltrain': 
                self.dataset = EventsVoxelDataset(base_folder, event_folder, depth_folder, start_time, stop_time,
                                                           clip_distance, every_x_rgb_frame, transform,
                                                           normalize=normalize, use_phased_arch=use_phased_arch,
                                                           baseline=baseline, loss_composition=loss_composition,
                                                           reg_factor=reg_factor)
        elif dataset_type == 'shifttrain':
                self.dataset = SynchronizedFramesContinuousEventsShiftedDataset(base_folder, event_folder, depth_folder, frame_folder,
                                                        start_time, stop_time, clip_distance, every_x_rgb_frame, 
                                                        transform, normalize=normalize, use_phased_arch=use_phased_arch,
                                                        baseline=baseline, loss_composition=loss_composition, shift_duration=shift_duration)
        else:
    ...
    def __getitem__(self, i):
        ...
        for n in range(self.L - 1):
            ...
            paused = (np.random.rand() < probability_pause)
            if paused:
        ...
        # down sample data : repulsive looking code!!!!!!!!!
...
if __name__ == '__main__':
    ...
    dataset1 = SynchronizedFramesEventsRawDataset(base_folder='./data/test/', event_folder='events/data/', depth_folder = 'depth/data/', frame_folder='rgb/frames/')
    dataloader1 = DataLoader(dataset1)
    d1 = iter(dataloader1).next()

    dataset2 = SynchronizedFramesEventsVoxelDataset(base_folder='./data/test/', event_folder='events/voxels/', depth_folder = 'depth/data/', frame_folder='rgb/frames/')
    dataloader2 = DataLoader(dataset2)
    d2 = iter(dataloader2).next()


    dataset3 = EventsVoxelDataset(base_folder='./data/test/', event_folder='events/data/', depth_folder = 'depth/data/')
    dataloader3 = DataLoader(dataset3)
    d3 = iter(dataloader3).next()
